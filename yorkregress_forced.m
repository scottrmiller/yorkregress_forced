% function [b,sigma_b,r,p,t,chi_square,sigma_chi,p_chi,i] = yorkregress_forced(X,Y,sigma_X,sigma_Y,ri)
...
%     York et al. (2004) and standard deviation of chi-square from Wehr and
%     Saleska (2017).  For a Matlab code of the general York regression not
%     forced through the origin or forced through one of the datapoints,
%     see, for example, Thirumalai et al. (2011).
%
% Input arguments
...
%     t             t statistic
%     chi_square    Chi square statistic (goodness-of-fit), or mean
%                   weighted squared deviation (MSWD)
%     sigma_chi     Standard deviation of chi square
%     p_chi         p value for chi-square goodness-of-fit test, for H0
%                   that the observed data are well represented by a linear
%                   relationship.  Values > significance level support
%                   linear relationship.
...
%% Calculate goodness-of-fit parameter (chi-square)  
% Reduced weighted chi-square, or mean squared weighted deviation (MSWD),
% is calculated following York et al. (2004); see also Wehr and Saleska
% (2017).  In the case here where slope is the only free parameter, there
% are n-1 degrees of freedom (Mahon, 1996). If the data are well
% represented by a straight-line model through the origin, the reduced
% weighted chi-square is expected to have a value about 1 +/-2 standard
% errors (Wehr and Saleska, 2017).  Deviations significantly different from
% one indicate that the variables x and y are not well related by a
% straight line and/or the assigned errors are incorrect (possibly not
% normally distributed).
S = sum((Wi).*(Y - b.*X).^2);   % Weighted sum of deviations from best-fit 
                                % line
chi_square = S/(n-1);           % Reduced weighted chi-square
sigma_chi = sqrt(2/(n-1));      % Standard deviation in the weighted 
                                % reduced chi squared statistic (Wehr and
                                % Saleska, 2017).  Standard error of the
                                % mean chi-square is sigma-chi/sqrt(n)
...
p_chi_left = chi2cdf(chi_square,n-1);    
p_chi_right = chi2cdf(chi_square,n-1,'upper'); 
